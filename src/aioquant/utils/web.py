...
class Websocket:
    ...
    Attributes:
        ...
        error_callback: You can use this param to specify a async callback function when you initializing Websocket
            module. `error_callback` is like `async def on_error_callback(error: Error): pass`
            and this callback function will be executed asynchronous when some error occur while ws module is running.
    """

    def __init__(self, url, connected_callback=None, process_callback=None, process_binary_callback=None,
                 check_conn_interval=10, error_callback=None):
        """Initialize."""
        self._url = url
    ...
    async def _connect(self) -> None:
        ...
        try:
            ...
            e = Error("connect to Websocket server error! url: {}".format(self._url))
    ...
    async def _receive(self):
        ...
        async for msg in self.ws:
            if msg.type == aiohttp.WSMsgType.TEXT:
                ...
                e = Error("receive event CLOSED: {}".format(msg))
                logger.error(e, caller=self)
                if self._error_callback:
                    SingleTask.run(self._error_callback, e)
            elif msg.type == aiohttp.WSMsgType.ERROR:
                e = Error("receive event ERROR: {}".format(msg))
                logger.error(e, caller=self)
                if self._error_callback:
                    SingleTask.run(self._error_callback, e)
            else:
    ...
    async def _check_connection(self, *args, **kwargs) -> None:
        ...
        if not self.ws:
            e = Error("Websocket connection not connected yet!")
            logger.warn(e, caller=self)
            if self._error_callback:
                SingleTask.run(self._error_callback, e)
            return
        if self.ws.closed:
            e = Error("Websocket connection was closed!")
            logger.error(e, caller=self)
            if self._error_callback:
                SingleTask.run(self._error_callback, e)
            SingleTask.run(self.reconnect)
    ...
    async def send(self, data) -> bool:
        ...
        if isinstance(data, dict):
            ...
            e = Error("send message failed: {}".format(data))
            logger.error(e, caller=self)
            if self._error_callback:
                SingleTask.run(self._error_callback, e)
            return False
...
class AsyncHttpRequests(object):
    ...
    async def fetch(cls, method, url, params=None, body=None, data=None, headers=None, timeout=30, **kwargs):
        ...
        try:
            ...
            logger.debug("response data is not json format!", caller=cls)
    ...
    @classmethod
    @async_method_locker("AsyncHttpRequests.close.locker", timeout=5)
    async def close(cls):
