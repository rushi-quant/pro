...
def initMongodb(host: str = "127.0.0.1", port: int = 27017, username: str = None, password: str = None,
                dbname: str = "admin"):
...
class MongoDBBase:
    ...
    def new_cursor(self, db: str, collection: str):
    ...
    async def get_list(self, spec: dict = None, fields: dict = None, sort: list = None, skip: int = 0, limit: int = 9999,
                       cursor=None):
    ...
    async def find_one(self, spec: dict = None, fields: dict = None, sort: list = None, cursor=None):
        """Get one document.
    ...
    async def count(self, spec: dict = None, cursor=None):
    ...
    async def update(self, spec: dict, update_fields: dict, upsert: bool = False, multi: bool = False, cursor=None):
        """ Update (a) document(s).
        ...
        if "_id" in spec:
            spec["_id"] = self._convert_id_object(spec["_id"])
        if update_fields.get("$set"):
            update_fields["$set"] = update_fields.get("$set")
        if update_fields.get("$unset"):
            update_fields["$unset"] = update_fields.get("$unset")
        if not multi:
    ...
    async def delete(self, spec: dict, cursor=None):
    ...
    async def remove(self, spec: dict, multi: bool = False, cursor=None):
    ...
    async def distinct(self, key: str, spec: dict = None, cursor=None):
        """ Distinct query.
    ...
    async def find_one_and_update(self, spec: dict, update_fields: dict, sort: list = None, upsert: bool = False,
                                  return_document: bool = False, fields: dict = None, cursor=None):
        ...
        result = await cursor.find_one_and_update(spec, update_fields, sort=sort, projection=fields, upsert=upsert,
                                                  return_document=return_document)
        if result and "_id" in result:
            result["_id"] = str(result["_id"])
        return result

    async def find_one_and_delete(self, spec: dict, fields: dict = None, cursor=None):
