...
class TA:
    ...
    @staticmethod
    def TD(item,records,start=4,kline_num=9):
        """
        自定义TD
        rushi 2022-01-13 17:30
        :param item:
        :param records:
        :param start:
        :param kline_num:
        ...
        if records:
            ...
            if records_len >= 100:
                a = 0
                b = 0
                for i in range(records_len - kline_num,records_len):
                    if records[i]['Close'] > records[i-start]['Close']:
                        a = a + 1
                    else:
                        a = 0
                    if records[i]['Close'] < records[i-start]['Close']:
